...
from schemas import ShelterCreate, ShelterUpdate, LocationPoint
from typing import List, Optional, Tuple
from fastapi import HTTPException

def location_to_wkt(location: LocationPoint) -> str:
    """Convierte LocationPoint a formato WKT para PostGIS"""
    return f"SRID=4326;POINT({location.longitude} {location.latitude})"

def parse_location_from_db(db: Session, shelter: Shelter) -> LocationPoint:
    """Extrae las coordenadas de la geometría PostGIS"""
    if not shelter.location:
        return None
    
    result = db.execute(
        text("SELECT ST_X(:geom::geometry) as lon, ST_Y(:geom::geometry) as lat"),
        {"geom": str(shelter.location)}
    ).first()
...
def get_shelters(
    ...
    if description_filter:
        query = query.filter(Shelter.description.ilike(f"%{description_filter}%"))
